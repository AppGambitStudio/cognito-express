...
class CognitoExpress {
  ...
  init() {
    return new Promise(async (resolve, reject) => {
      try {
        console.log("[cognito-express] creating the validation keys");
        const response = await axios(`${this.iss}/.well-known/jwks.json`);
        console.log("[cognito-express] keys response", response);
        if (response.data.keys) {
          ...
          console.log("[cognito-express] no keys found");
        }
      } catch (err) {
        console.log("cognito-express error", err.message);
        console.error(err);
        reject("[cognito-express] Unable to generate certificate due to \n" + err);
      }
    });
  }

  async validate(token, callback) {
    console.log("[cognito-express] before token validation");
    await this.hasFinishedProcessing;
    console.log("[cognito-express] starting token validation");
    return new Promise(async (resolve, reject) => {
